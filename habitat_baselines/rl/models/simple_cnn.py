...
class MapCNN(nn.Module):
    ...
    def __init__(self, observation_space, output_size):
        super().__init__()
        if "agent_map" in observation_space.spaces:
            self._n_input_map = 1
        else:
            self._n_input_map = 0
        ...
        if self._n_input_map > 0:
            cnn_dims = np.array(
                observation_space.spaces["agent_map"].shape[:2], dtype=np.float32
            )
    ...
    def forward(self, observations):
        if self._n_input_map > 0:
            shape_map=observations["agent_map"].shape
            map_observations = observations["agent_map"].view(shape_map[0], shape_map[1], shape_map[2], 1)
            map_observations = map_observations.permute(0, 3, 1, 2)
            map_observations = map_observations / 255.0  # normalize Map
