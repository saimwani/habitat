...
def draw_top_down_map(info, output_size=256):
    ...
    top_down_map[top_down_map<255]=144
    ...
    pos=info["top_down_map"]["agent_map_coord"]
    patch=top_down_map[map_agent_pos[0]+top-50:map_agent_pos[0]+top+50, map_agent_pos[1]+left-50:map_agent_pos[1]+left+50]
    # img=Image.fromarray(top_down_map)
    # img.show()
    # img=Image.fromarray(patch)
    # img.show()
    mapDict[info["episode_info"]["scene_id"]]=top_down_map
    return patch
...
class PPOTrainer(BaseRLTrainer):
    ...
    def _collect_rollout_step(
        ...
        for i in range(len(observations)):  ##CHNG
            observations[i]["agent_map"]=draw_top_down_map(infos[i], 256)
    ...
    def train(self) -> None:
        ...
        batch = batch_obs(observations)
         ...
         #img=Image.fromarray(top_down_map, 'RGB');img.show()
        ...
        with TensorboardWriter(
            ...
            for update in range(self.config.NUM_UPDATES):
                ...
                # log stats
                if update > 0 and update % 10 == 0:  ##CHNG: if update > 0 and update % self.config.LOG_INTERVAL == 0:
                    logger.info(
    ...
    def _eval_checkpoint(
        ...
        metric_name = self.config.TASK_CONFIG.TASK.MEASUREMENTS[1]
